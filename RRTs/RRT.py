...
#Same as Random trees except we add a bias to the node
...
class RRT:
    ...
    def generate_next_point(self, bias):
    ...
    def generate_graphs(self, bias):
        ...
        while (not self.end_generation):
            last_point = self.generate_next_point(bias)
            if last_point:
                # Plot the new point
...
rrt = RRT(0.5, start_node, end_node, grid)
final_trajectory = rrt.generate_graphs(0.9)[1]
